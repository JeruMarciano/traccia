...
| **No LICENSE file, no README** | Still open from Phase 1, but **not urgent: the repository is private** (checked 2026-08-01). Absent a licence, all rights are reserved — which only starts to matter, and then contradicts the intent to let anyone try the tool, at the point the repository is made public or binaries are attached to a release. A release-readiness item, not a prerequisite for further work. |
